...
Referring Expression Comprehension is the task of grounding a natural-language expression in an image, usually by identifying the object or region the expression refers to.
...
> REC is a grounding task where the model must map language to a specific region, object, or field. I think about it as a conditional localization problem, often evaluated with IoU-thresholded accuracy. It is important because it exposes whether the model really grounds language in the visual scene, which is especially relevant for document systems and fine-grained multimodal assistants.
