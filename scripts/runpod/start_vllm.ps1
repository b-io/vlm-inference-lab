...
$CHECK_MODEL = if ($SERVED_MODEL_NAME)
{
    $SERVED_MODEL_NAME
}
else
...
while ($true)
{
    ...
    catch
    {
        ...
        if (Get-Command vllm -ErrorAction SilentlyContinue)
        {
            if (-not (Get-Process -Id $VLLM_PID -ErrorAction SilentlyContinue))
            {
                ...
                if (Test-Path vllm_server.err)
                {
                    Get-Content vllm_server.err | Select-Object -Last 20
                }
                exit 1
            }
        }
    ...
    if ($RETRY_COUNT -ge $MAX_RETRIES)
    {
        ...
        if (Get-Command vllm -ErrorAction SilentlyContinue)
        {
            if (Test-Path vllm_server.log)
            {
                Get-Content vllm_server.log | Select-Object -Last 20
            }
        }
        else
        {
