...
if (-not $env:RUNPOD_BASE_URL)
{
    Write-Error "RUNPOD_BASE_URL is required for both modes"; exit 1
}
if (-not $env:MODEL_ID)
{
    Write-Error "MODEL_ID is required"; exit 1
}

# New Configuration Variables
$SSH_STRATEGY = if ($env:RUNPOD_SSH_MODE)
{
    $env:RUNPOD_SSH_MODE
}
else
...
$ALLOW_MUTATION = if ($env:RUNPOD_ALLOW_MANAGED_INSTANCE_MUTATION -eq "true")
{
    $true
}
else
...
function Get-IsManagedInstance($baseUrl, $remoteTarget, $sshKey, $sshPort)
{
    ...
    if ($remoteTarget -and $sshKey)
    {
        ...
        if ($out -ne "NOT_FOUND" -and $out -match "\d+")
        {
            return $true
        }
    }
...
if ($MODE -eq "generic")
{
    ...
    if (-not $env:RUNPOD_SSH_PORT)
    {
        $env:RUNPOD_SSH_PORT = "22"
    }
    if (-not $env:RUNPOD_SSH_KEY_PATH)
    {
        $env:RUNPOD_SSH_KEY_PATH = "~/.ssh/id_ed25519"
    }
}
...
if ($MODE -eq "generic")
{
    ...
    if ($SSH_STRATEGY -eq "proxied")
    {
        ...
        {
            Write-Error "RUNPOD_PROXY_SSH_TARGET is required for proxied strategy"; exit 1
        }
    ...
    {
        Write-Error "Failed to deploy startup script"; exit 1
    }

    Write-Host "Starting vLLM remotely..."
    $ALLOW_KILL = if ($env:RUNPOD_ALLOW_KILL_EXISTING_VLLM -eq "true")
    {
        "true"
    }
    else
    ...
    if ($LASTEXITCODE -ne 0)
    {
        Write-Error "Remote startup script failed"; exit 1
    }
}
...
Write-Host "--------------------------------------------------------"
Write-Host "Tier        : $( if ($env:BENCHMARK_TIER)
{
    $env:BENCHMARK_TIER
}
else
