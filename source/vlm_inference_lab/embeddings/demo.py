...
def generate_mock_embedding(dim: int = 128) -> List[float]:
    """Generates a random unit-length embedding."""
...
def run_vlm_demo():
    """
    Simulates image-text alignment using mock embeddings.
    Shows how a 'query' text is compared against multiple 'candidate' images.
    
    This illustrates the 'Dual Encoder' architecture (like CLIP), where 
    text and images are projected into a shared high-dimensional space.
    """
    dim = 128
    ...
    images = [
        ("image_dog.jpg", generate_mock_embedding(dim)),
        ...
        ("image_car.jpg", generate_mock_embedding(dim)),
        ("image_forest.jpg", generate_mock_embedding(dim)),
    ]
    ...
    print("\nInterview Note: In generative VLMs (like LLaVA), visual tokens are often ")
